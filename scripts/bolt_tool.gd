...
func place_bolt(world_position: Vector2) -> void:
	...
	if layer1_body == null and layer2_body == null:
		print("No objects found at bolt position. Exit cursor/toolbox mode to convert preview objects to physics objects first.")
		return
	...
	pin_joint.softness = 0.0  # Rigid connection
	pin_joint.disable_collision = false  # Allow collision between connected bodies
